...
class FailureAlertView: UIView {
    ...
    init(frame:CGRect, title:String,alertType:String,msgToShow:String,amount : String,buttonsNameDic:[String] = ["CANCEL","RETRY"]) {
        super.init(frame: frame)
        
        self.backgroundLayer = UIView(frame: (self.bounds));
        self.backgroundLayer.backgroundColor =  UIColor.white
        self.layer.cornerRadius = 20
        self.layer.borderWidth = 2
        self.layer.borderColor = UIColor.init(red: 89/255, green: 41/255, blue: 178/255, alpha: 1.0).cgColor
        ...
        //self.backgroundColor = UIColor.init(red: 216/255, green: 212/255, blue: 209/255, alpha: 1.0)
         ...
         let heightOfText = CommonFunctions.sharedInstance.calculateHeight(inString: msgToShow, width:self.frame.width - 80, font: UIFont(name: CustomFont.helveticaRegularFont, size: 15.0)!) + 80
        ...
        var imgIconName = "thumb_down"
        
        let alertImg = UIImage(named: "thumb_down")?.scaleTo(CGSize(width: 160, height: 160))
        
        var defaultTitle = "Alert !"
        switch alertType {
        case AppUtils.Success: imgIconName = "thumb_down";
        defaultTitle = "Success !"
        break;
        case AppUtils.ErrorTypeTitle :  imgIconName = "thumb_down"
        defaultTitle = "Alert !"
        break;
        case AlertType.error.rawValue : imgIconName = "thumb_down"
                                        defaultTitle = "Error !"
        break;
        case AlertType.info.rawValue : imgIconName = "thumb_down"
                                       defaultTitle = "Info"
        case AlertType.tellus.rawValue:
            imgIconName = "thumb_down"
            defaultTitle = ""
        break;
        ...
        if title != "" {
            ...
            headingLbl.font = UIFont(name: "Montserrat-Bold", size: 18.0)
            ...
            let msg = AppUtils.rupeeSymbol + String(Int(amount)!/100) + "\n" + msgToShow
            msgLbl.text = msg;
            msgLbl.textAlignment = .center;
            msgLbl.numberOfLines = 0
            msgLbl.lineBreakMode = .byWordWrapping
            msgLbl.font = UIFont(name: "Montserrat-Bold", size: 18.0)
            ...
            let attrs2 = [NSAttributedString.Key.font : UIFont(name: "Montserrat-Bold", size: 18), NSAttributedString.Key.foregroundColor : UIColor.darkGray]
        ...
        okBtn = UIButton(frame: CGRect(x: 20, y: popUpView.frame.height - 60, width: 120, height: 40))
        ...
        okBtn.titleLabel?.font = UIFont(name: "Montserrat-Bold", size: 14)
        popUpView.addSubview(okBtn)
        
        retryBtn = UIButton(frame: CGRect(x: (popUpView.frame.width - 140), y: popUpView.frame.height - 60, width: 120, height: 40))
        retryBtn.setTitle(buttonsNameDic[1], for: .normal)
        retryBtn.setTitleColor(UIColor.white, for: .normal)
        retryBtn.backgroundColor = UIColor.init(red: 89.0/255.0, green: 41.0/255.0, blue: 178.0/255.0, alpha: 1.0)
        retryBtn.layer.cornerRadius = retryBtn.frame.size.height/2
        retryBtn.clipsToBounds = true
        retryBtn.titleLabel?.font = UIFont(name: "Montserrat-Bold", size: 14)
        popUpView.addSubview(retryBtn)
